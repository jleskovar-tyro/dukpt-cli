...
class Thales {
  constructor(host, port) {
    this.host = host
    this.port = port
    this.debug = process.env.DUKPT_DEBUG || false
  }
  ...
  async generateMac(bdk, ksn, data) {
    ...
    if (dataBuffer.length % 8 > 0) {
      dataBuffer = Buffer.concat([dataBuffer, Buffer.alloc(8 - (dataBuffer.length % 8), 0)])
    }
    
    proxy.length = request.length + dataBuffer.length - 2
    proxy.messageLength = dataBuffer.length.toString().padStart(4, '0')
    
    request = Buffer.concat([request, dataBuffer])
  ...
  async encrypytData(bdk, ksn, data) {
    ...
    proxy.length = request.length + dataBuffer.length - 2
    proxy.messageLength = dataBuffer.length.toString(16).padStart(4, '0')
